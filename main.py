...
api_name="youtube"
api_version="v3"
api_key="enter your youtube api key" # youtube api key
...
with st.sidebar:
    selected = option_menu("Menu",["Home","Database","query","Visualization"], orientation="vertical")
    ...
    if st.button("Upload to MYSQL database"):  # upload the youtube retrieved data into MYSQL database
        ...
        with st.spinner('Upload in progress...'):
            try:
                ...
                cursor.execute('''create table if not exists channel( channel_name VARCHAR(100) ,
                ...
                cursor.execute('''create table if not exists playlist(playlist_id VARCHAR(50) PRIMARY KEY,playlist_name VARCHAR(100),
                ...
                cursor.execute('''create table if not exists videos(channel_id VARCHAR(50),video_id VARCHAR(50)primary key,
                ...
                cursor.execute('''create table if not exists comments(video_id VARCHAR(50),comment_id VARCHAR(50),comment_text TEXT,
...
def load_channel_data(channel_name):
    # Fetch channel data
    cursor.execute("SELECT * FROM channel WHERE channel_name = %s", (channel_name,))
    out = cursor.fetchall()
    channel_df = pd.DataFrame(out, columns=[i[0] for i in cursor.description]).reset_index(drop=True)
    channel_df.index += 1

    # Fetch playlists data
    cursor.execute("SELECT * FROM playlist WHERE channel_id = %s", (channel_df['channel_id'].iloc[0],))
    out = cursor.fetchall()
    playlists_df = pd.DataFrame(out, columns=[i[0] for i in cursor.description]).reset_index(drop=True)
    playlists_df.index += 1

    # Fetch videos data
    cursor.execute("SELECT * FROM videos WHERE channel_id = %s", (channel_df['channel_id'].iloc[0],))
    out = cursor.fetchall()
    videos_df = pd.DataFrame(out, columns=[i[0] for i in cursor.description]).reset_index(drop=True)
    videos_df.index += 1

    # Fetch comments data
    cursor.execute("SELECT * FROM comments WHERE video_id IN (SELECT video_id FROM videos WHERE channel_id = %s)",
                     (channel_df['channel_id'].iloc[0],))
    out = cursor.fetchall()
    comments_df = pd.DataFrame(out, columns=[i[0] for i in cursor.description]).reset_index(drop=True)
    comments_df.index += 1
...
def sql_question_1():
    cursor.execute('''SELECT channel.channel_name,videos.video_name
                        FROM videos 
...
def sql_question_2():
    cursor.execute('''SELECT DISTINCT channel_name,COUNT(videos.video_id) as Total_Videos 
                        FROM channel 
...
def sql_question_3():
    cursor.execute('''SELECT channel.Channel_name,videos.Video_name, videos.View_Count as Total_Views
                        FROM videos
                        JOIN channel ON channel.Channel_id = videos.Channel_id
...
def sql_question_4():
    cursor.execute('''SELECT videos.video_name,videos.comment_count as Total_Comments
                    FROM videos
...
def sql_question_5():
    cursor.execute('''SELECT channel.channel_name,videos.video_name,videos.like_count as Highest_likes FROM videos 
...
def sql_question_6():
    cursor.execute('''SELECT videos.video_name,videos.like_count as Likes
                    FROM videos
...
def sql_question_7():
    cursor.execute('''SELECT channel.channel_name,channel.channel_views as Total_views
                    FROM channel
...
def sql_question_10():
    cursor.execute('''SELECT channel.channel_name,videos.video_name,videos.comment_count as Total_Comments
                    FROM videos
                    JOIN channel ON channel.channel_id=videos.channel_id
                    ORDER BY videos.comment_count DESC''')
    out = cursor.fetchall()
...
if selected == "Visualization":
    ...
    if Option == '1.Channels with Subscriber Count':
        with st.spinner('Ploting in progress...'):
            def plot_ques_1():
                cursor.execute('''SELECT channel_name,channel_subscribers 
                                    FROM channel
                                    ORDER BY channel_subscribers DESC''')
                out = cursor.fetchall()
                df = pd.DataFrame(out, columns=['Channel Name', 'Subscribers Count'])
                fig = px.bar(df, x='Channel Name', y='Subscribers Count', color='Channel Name',
    ...
    if Option == '2.Channels with highest No Of Videos':
        def plot_ques_2():
            cursor.execute('''SELECT channel_name,channel_video_count as Total_Videos 
                            FROM channel 
    ...
    if Option == '6.Year wise Performance of each Channel':
        def plot_ques_6():
            cursor.execute('''SELECT DISTINCT Year(videos.publishedAt) AS Years, COUNT(videos.video_id) AS Total_videos,
            ...
            out = cursor.fetchall()
            df = pd.DataFrame(out,
                              columns=['Years', 'Total videos', 'Likes', 'Views', 'Total Comments', 'Channel Name'])
